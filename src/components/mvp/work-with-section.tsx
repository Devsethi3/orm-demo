...
const workData: WorkItem[] = [
  {
    ...
    description:
      "Turning ideas into structured, buildable products with clear direction.",
    icon: "/icons/bulb.svg",
  },
  {
    title: "Non-Technical Founders",
    description:
      "Bringing technical execution, architecture, and product clarity from day one.",
    icon: "/icons/users.svg",
  },
  {
    title: "Pre-Seed / Seed Startups",
    description:
      "Moving from concept to investor-ready MVP with speed and structure.",
    icon: "/icons/trend.svg",
  },
  {
    title: "Founders Validating Ideas",
    description:
      "Building fast, focused MVPs to test real market demand.",
...
const WhoWeWorkWith: React.FC = () => {
  return (
    <section className="bg-black text-white py-16 sm:py-20 md:py-24 px-3 sm:px-6 lg:px-12 w-full flex flex-col items-center selection:bg-white selection:text-black">
      ...
      <div className="w-full max-w-[1400px] relative border-y border-[#333]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 divide-y md:divide-y-0 md:divide-x divide-[#333] bg-[#0c0c0c]">
          {workData.map((item, index) => (
            <div
              key={index}
              className="flex flex-col p-6 sm:p-8 lg:p-10 min-h-[260px] sm:min-h-[280px] bg-[#0c0c0c]"
            >
              <Image
                src={item.icon}
                alt={item.title}
