...
export default function ExecutionPathsSection() {
  return (
    <section className="relative w-full min-h-screen flex flex-col items-center py-24 px-3 lg:px-12 bg-black text-white selection:bg-white selection:text-black overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 w-full items-stretch">
          {pathsData.map((card, index) => (
            <div key={index} className="flex h-full">
              <CornerBrackets>
                <div
                  className={`flex flex-col h-full p-8 lg:p-10 transition-colors duration-300
                    ${
                      card.isHighlighted
                        ? "bg-white text-black"
