...
const TechCategoryCard = ({
  ...
  return (
    <div
      className={clsx(
        "group relative h-full rounded-xl border border-zinc-800 bg-zinc-900/20 p-6 transition-all duration-300 hover:border-zinc-700 hover:bg-zinc-900/50",
        className, 
      )}
    >
      <BlueprintCorners />
      <div className="relative z-20 flex h-full flex-col">
        ...
        <div
          className={
            isFullWidth
              ? "grid grid-cols-2 gap-x-8 gap-y-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5"
              : "flex flex-col gap-4"
          }
        >
          {items.map((tech) => (
  ...
  return (
    <section className="flex w-full flex-col items-center bg-black px-4 py-24 text-white selection:bg-white selection:text-black md:px-8 lg:px-12">
      <div className="mb-20 flex max-w-4xl flex-col items-center text-center">
        ...
        <h2 className="font-heading text-4xl leading-[1.1] text-muted-foreground sm:text-5xl md:text-[4rem]">
          Our Trusted <br className="hidden md:block" />
